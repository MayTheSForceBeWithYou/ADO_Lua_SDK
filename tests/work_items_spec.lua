...
describe("work_items:get_updates", function()
  it("sends GET /{project}/_apis/wit/workItems/{id}/updates", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_updates(42, { project = "P" })
    assert.equals("GET", c._last_req.method)
    assert.equals("/P/_apis/wit/workItems/42/updates", c._last_req.path)
  end)

  it("returns nil + err when project is absent", function()
    local c = make_client()
    local m = wi_factory(c)
    local _, err = m:get_updates(1, {})
    assert.is_not_nil(err)
    assert.truthy(err.message:match("project"))
  end)

  it("forwards extra query params (e.g. $top)", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_updates(5, { project = "P", ["$top"] = 10 })
    assert.equals(10, c._last_req.params["$top"])
  end)

  it("does not leak project into query params", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_updates(7, { project = "MyProj" })
    assert.is_nil(c._last_req.params.project)
  end)
end)

describe("work_items:get_comments", function()
  it("sends GET /{project}/_apis/wit/workItems/{id}/comments", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_comments(42, { project = "P" })
    assert.equals("GET", c._last_req.method)
    assert.equals("/P/_apis/wit/workItems/42/comments", c._last_req.path)
  end)

  it("pins comments api-version and orders newest first", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_comments(1, { project = "P" })
    assert.equals("7.0-preview.3", c._last_req.params["api-version"])
    assert.equals("desc", c._last_req.params.order)
  end)

  it("returns nil + err when project is absent", function()
    local c = make_client()
    local m = wi_factory(c)
    local _, err = m:get_comments(1, {})
    assert.is_not_nil(err)
    assert.truthy(err.message:match("project"))
  end)
  ...
  it("does not leak project into query params", function()
    local c = make_client()
    local m = wi_factory(c)
    m:get_comments(7, { project = "MyProj" })
    assert.is_nil(c._last_req.params.project)
  end)
end)

describe("work_items:add_comment", function()
  it("sends POST /{project}/_apis/wit/workItems/{id}/comments", function()
    local c = make_client()
    local m = wi_factory(c)
    m:add_comment(42, "hello", { project = "P" })
    assert.equals("POST", c._last_req.method)
    assert.equals("/P/_apis/wit/workItems/42/comments", c._last_req.path)
    assert.same({ text = "hello" }, c._last_req.body)
    assert.equals("7.0-preview.3", c._last_req.params["api-version"])
  end)

  it("returns nil + err when text is empty", function()
    local c = make_client()
    local m = wi_factory(c)
    local _, err = m:add_comment(1, "   ", { project = "P" })
    assert.is_not_nil(err)
    assert.truthy(err.message:match("text"))
  end)

  it("returns nil + err when project is absent", function()
    local c = make_client()
    local m = wi_factory(c)
    local _, err = m:add_comment(1, "hi", {})
    assert.is_not_nil(err)
    assert.truthy(err.message:match("project"))
  end)
end)
